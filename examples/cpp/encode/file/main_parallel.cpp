...
/*
 *
 * This encoder is based on the libFLAC++ library encoder example included with
 * the FLAC source. It is somewhat limited, supporting only 16-bit stereo files
 * in canonical WAVE format.
 *
 * It also currently disables:
 *      Verification while encoding
 *      MD5 verification after encoding
 *      Loose mid-side stereo channel splitting
 *      Progress callbacks are limited, but still work for the most part
 *
 * The good news is that it runs it parallel, potentially using all processor
 * cores available.
 *
 */
...
int main(int argc, char *argv[])
{
    ...
    if(ok) {
        ...
        InputFilter input_filter(fin, channels*(bps/8), left, &encoder, 5);
        pl.add_filter(input_filter);

        PCMFilter pcm_filter;
        pl.add_filter(pcm_filter);

        ProcessFilter process_filter;
        pl.add_filter(process_filter);

        OutputFilter output_filter;
        pl.add_filter(output_filter);

        pl.run(InputFilter::num_tokens);
